...
| [Beyond the ATE](https://www.beyondtheate.com/) | Nick Williams（UC Berkeley 生统博士生 / Columbia Mailman）、Kara Rudolph（Columbia Mailman 流病）、Iván Díaz（NYU Grossman 生统） | 免费在线 · [源码](https://github.com/nt-williams/lmtp-workshop) | `R` `workshop` `MTP` `lmtp` `quarto` `webR` `GPL-3` | 待读 | 与软件包里的 [lmtp](https://github.com/nt-williams/lmtp) 成对收：那条是包，这条是它唯一的正经学习入口（lmtp 没有 pkgdown 站）。11 章按“先定义估计量、再讲估计器、最后上手”的顺序走：Introduction → Defining Interventions → Estimators，然后是 lmtp 实操六节（静态效应与因果对比、动态方案 DTR、MTP、IPSI、生存分析、多元暴露）。最大的好处是 Quarto + webR，R 直接在浏览器里跑，不用先在本机装 lmtp 和 SuperLearner 就能过一遍。三位讲师分工正好覆盖三块：Williams 是包作者，Díaz 是方法学论文作者，Rudolph 做可迁移性与中介。定位要清楚：这是会议讲义不是教科书，理论推导的深度远不如 JASA 那篇，胜在能当天上手；只想看方法学定位、不碰代码的直接读 arXiv:2304.09460。仓库 [nt-williams/lmtp-workshop](https://github.com/nt-williams/lmtp-workshop) 协议是 GPL-3，和 lmtp 包本身的 AGPL-3 不是一个，引用讲义内容时按 GPL-3 对。从 SER 2024（Austin）一路讲到 ACIC 2026（盐湖城）、SER 2026（凤凰城）、SLDS 2026（布鲁克林），最后提交 2026-06，仍在随包更新。 |
...
| 名称 | 描述 | 标签 | 状态 | Star | 批注 |
|---|---|---|---|---|---|
| [video-transcribe-turbo（本地 Skill）](https://github.com/openai/whisper) | 基于本地 Whisper turbo 的批量视频 / 音频转写流程，生成 TXT、SRT、JSON，支持 CUDA、断点续跑与输出一致性验证 | `Python` `local-skill` `Whisper` `turbo` `TXT` `SRT` `JSON` `CUDA` | 在用 | 109,468 | 本机已安装 video-transcribe-turbo；默认中文用 turbo / zh，脚本一次加载模型并支持 resume 与 validate。它解决批量转写流程，不等同于逐字人工校对，也不负责画面 OCR。 |
...
| [PaperSpine](https://github.com/WUBING2023/PaperSpine) · [文档](https://wubing2023.github.io/PaperSpine/v5/) | 论文全流程 skill（PaperSpine5）：给研究方向、资料或实验数据，它查文献、梳论点、搭大纲、写全文、生成科研配图，再做引用核验、审阅与排版，交付可编辑 Word / LaTeX 与 PDF；材料本地优先，论点与引用要求有证据 | `paper-writing` `local-first` `LaTeX` `Word` `MIT` | 待读 | 5,558 | 收藏时还是 v3 / v4「以动机为导向、修订矩阵、LaTeX 安全审核」的定位，现在是带 Web 工作区的自包含套件（Windows 约 26 MB，v0.4.0 alpha 预发布，未签名），装到 codex 或 claude 宿主。工具链里的 writing_rationale_matrix、citation_support_bank、latex_guard 可以单独看。 |
| [Auto-Empirical Research Skills（AERS）](https://github.com/brycewang-stanford/Auto-Empirical-Research-Skills) · [文档](https://copaper.ai) | 斯坦福 REAP × CoPaper.AI 的社会科学实证研究 skill 库：76 个合集、1,096 个 skill，9 阶段流水线从数据清洗到顶刊投稿，Python / Stata / R + Quarto 三条计量流水线，带数值基准与行为评测 | `econometrics` `DID` `stata` `quarto` `CC-BY-SA` | 待读 | 4,135 | 计量经济学口味：DID、Callaway–Sant'Anna 事件研究、HonestDiD 是它的主场，与因果推断板块里 DID / 合成控制的内容互补。GitHub 显示协议为 Other，实际 LICENSE 是 CC BY-SA 4.0。可通过插件市场只装 empirical-analysis-r 这一条线。 |
| [AI Research SKILLs（Orchestra）](https://github.com/Orchestra-Research/AI-Research-SKILLs) · [文档](http://orchestra-research.com) | Orchestra Research 的 AI 研究工程 skill 库：98 个 skill、23 类，autoresearch 双循环编排从选题、文献到实验与论文写作，其余是 Megatron、vLLM、TRL、微调、RAG、评测等训练与部署知识 | `ML-research` `autoresearch` `paper-writing` `MIT` | 待读 | 12,942 | 面向 ML 研究者，与临床统计关系不大；留它是看 autoresearch 的两层循环怎么把 ideation、ML paper writing 与实验 skill 编排起来。npx 安装器把 skill 装到 ~/.orchestra/skills 再软链到各 agent，Windows 退化为复制。 |
